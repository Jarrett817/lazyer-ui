<template>
  <template v-if="isMobile">
    <div class="ripple-container" ref="rippleElement" @touchstart="showRipple" >
    </div>
  </template>
  <template v-else>
    <div class="ripple-container" ref="rippleElement" @mousedown="showRipple" >
    </div>
  </template>
</template>
...
export default {
  ...
  setup() {
    ...
    const showRipple = (event) => {
      ...
      //获取元素的大小
      const pos = rippleElement.value.getBoundingClientRect();
      //获取点击坐标
      let clientX = 0;
      let clientY = 0;
      if (isMobile.value) {
      ...
      app.mount(rippleElement.value);
      setTimeout(() =>{
        //清除ripple
          app.unmount(rippleElement.value);
      }, 250)
    };
    return {
      showRipple, rippleElement, isMobile
    };
  }
};
</script>
